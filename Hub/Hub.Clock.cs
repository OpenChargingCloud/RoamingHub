...
namespace cloud.charging.open.RoamingHub
{
    ...
    /// <summary>
    /// What time this Hub thinks it is, and what that is worth.
    /// </summary>
    /// <remarks>
    /// Two different questions, and a Hub has to keep them apart.
    ...
    /// **this Hub does not set its clock from the answer**. It measures the
    /// difference and reports it.
    ///
    /// That is deliberate. A Hub stamps everything its roaming partners send
    /// it - the sessions, the charge detail records, the moment a token was
    /// authorised - against this clock, and those records are held against its
    /// customers' invoices, so stepping it is not something a background task
    /// does by surprise; a jump backwards would put a session's end before its
    /// start in a record written somewhere else entirely, and there would be
    /// nothing in it to say why. Measuring is the part that can be done safely
    /// and is the part that tells somebody whether there is a problem.
    ///
    ...
    /// <c>nts.legalTimeAuthority</c>, and this Hub then only repeats that
    ...
    /// says the time is unverified instead - which is a true statement about an
    /// Hub that has not checked, and the whole point of not
    ...
    public partial class Hub
    {
        ...
        /// <summary>
        /// How often this Hub checks its clock.
        /// </summary>
        ...
        /// <summary>
        /// How far this Hub's clock may be off and still count.
        /// </summary>
        ...
        /// <summary>
        /// Begin checking this Hub's clock against its time server.
        /// </summary>
        /// <remarks>
        /// Through the Hub's own <see cref="TimeProvider"/> rather than a
        ...
        private void StartCheckingTheClock()
        {
            ...
            if (!NTSEnabled)
            {
                Log.Info("The clock of this Hub is not being checked: NTS is switched off.", "nts", "clock");
                return;
            }
            ...
            Log.Info(
                $"The clock of this Hub will be checked against {ntsClient.Hostname} every {TimeCheckEvery.TotalMinutes:F0} minute(s)" +
        ...
        private async Task CheckTheClockAsync()
        {
            ...
            try
            {
                ...
                if (result.Value<Boolean>("ok") != true)
                    Log.Warning(
                        $"The clock of this Hub could not be checked: {result.Value<String>("error") ?? "no answer"}.",
                        "nts", "clock"
                    );
            ...
            {
                Log.Warning($"The clock of this Hub could not be checked: {e.Message}", "nts", "clock");
            }
        ...
        public JObject ClockJSON()
        {
            ...
            return new JObject(
                       ...
                       // Said out loud: the number above is this Hub's own
