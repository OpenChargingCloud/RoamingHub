...
namespace cloud.charging.open.RoamingHub
{
    ...
    /// </para>
    /// </remarks>
    public partial class Hub : IAsyncDisposable
    {
        ...
        /// share this server: the JSON API of this Hub at "/api", the web
        ...
        /// thing it stands for. Not "Hub": an organization identification has
        /// to be at least four characters, and a name that is refused at the
        /// first start is a program that does not start.
        /// </remarks>
        ...
        /// <summary>
        /// The name servers this Hub would ask, whether or not name
        /// resolution is switched on at the moment.
        /// </summary>
        private           IReadOnlyList<DNSServerConfig>  configuredDNSServers;

        /// <summary>
        /// Serialises changes to what this Hub is made of, so that two
        /// browsers saving at the same moment do not build half a Hub each.
        /// </summary>
        ...
        /// <summary>
        /// Where everything this Hub can be told in writing lives between
        /// starts: its name resolution, its time source, its OCPI identity.
        /// </summary>
        public HubConfigFile         ConfigFile             { get; }
        ...
        /// <summary>
        /// Whether this Hub resolves names at all.
        /// </summary>
        public Boolean                DNSEnabled             { get; private set; } = true;

        /// <summary>
        /// Whether this Hub may ask its time server.
        /// </summary>
        public Boolean                NTSEnabled             { get; private set; } = true;

        /// <summary>
        /// The password this Hub made up because there was no login file, or
        ...
        /// this Hub is administered with - and what the OCPI endpoints hang
        /// off.
        /// </summary>
        /// <remarks>
        ...
        /// <summary>
        /// Whether those accounts are this Hub's own, or somebody else's that
        /// it was handed.
        /// </summary>
        public Boolean                OwnsExtAPI             { get; }

        /// <summary>
        /// Whether the HTTP server is this Hub's own, or one it was handed
        /// and shares with somebody else.
        /// </summary>
        public Boolean                OwnsHTTPServer         { get; }

        /// <summary>
        /// Everything of this Hub - its web interface, its JSON API and,
        ...
        /// root, and "/Hub" or the like below one.
        /// </summary>
        ...
        /// see Hub.Clock.cs for why it is handed in rather than reached for.
        ...
        /// <summary>
        /// When this Hub was created, by its own clock.
        /// </summary>
        ...
        /// <summary>
        /// Create a Hub with a web interface in front of it.
        ...
        /// <param name="BasePath">What everything of this Hub sits below; the root by default. Something else only where several of these programs share one HTTP server.</param>
        /// <param name="HTTPRootPath">The root path of the JSON API, "/api" below <paramref name="BasePath"/> by default.</param>
        /// <param name="ExtAPI">An HTTPExt API to sign in against, or null for one of this Hub's own. Handing one in is what makes one sign-in open several of these programs at once.</param>
        /// <param name="HTTPExtAPIPath">Where this Hub's own HTTPExt API sits below <paramref name="BasePath"/>, "/ext" by default. Ignored when one is handed in.</param>
        ...
        /// <param name="ConfigFile">Where everything this Hub can be told in writing lives; "configuration.json" beside the process by default.</param>
        /// <param name="OCPI">Who this Hub is in OCPI, unless the configuration file says otherwise.</param>
        ...
        /// <param name="TimeProvider">Where this Hub reads the time; the system clock by default.</param>
        public Hub(DNSClient?             DNSClient               = null,
                    ...
                    TimeProvider?          TimeProvider            = null)
        {
            ...
            // the Hub it belongs to cannot be held against anything.
            ...
            this.Version      = typeof(Hub).Assembly.GetName().Version?.ToString(3) ?? "0.0.0";
            ...
            this.ConfigFile = ConfigFile ?? new HubConfigFile(HubConfigFile.DefaultFileName);
            ...
            if (this.ConfigFile.Exists)
            {
                ...
                // paper over with defaults: somebody wrote down what their Hub
            ...
            this.httpServer    = HTTPServer   ?? new HTTPServer(
                                                     IPAddress:       address,
                                                     TCPPort:         port,
                                                     HTTPServerName:  $"OpenChargingCloud Hub v{Version}",
                                                     DNSClient:       dnsClient
                                                 );

            this.BasePath      = BasePath     ?? HTTPPath.Root;

            this.httpRootPath  = HTTPRootPath ?? this.BasePath + HubHTTPAPI.DefaultAPIPath;
            ...
            this.ExtAPI        = ExtAPI ?? new HTTPExtAPI(
                                     ...
                                     HTTPServerName:         $"OpenChargingCloud Hub v{Version}",
                                     HTTPServiceName:        $"OpenChargingCloud Hub v{Version}",
                                     APIRobotEMailAddress:   EMailAddress.Parse("OpenChargingCloud Hub Robot <robot@charging.cloud>"),
                                     APIRobotGPGPassphrase:  "",

                                     // Nothing here sends mail. A Hub that
                                     ...
                                     // over plain HTTP, and a Hub on a bench
                                     // is reached over plain HTTP.
                                     UseSecureCookies:       false,

                                     // The shortest name a role of this Hub
            ...
            this.Log.Info(
                OwnsExtAPI
                    ...
                    : $"This Hub signs in against accounts it shares, at '{this.ExtAPI.RootPath}'.",
                "web", "http"
            );
            ...
            this.Frontend      = Frontend ?? new EmbeddedContentSource(HTTPRoot, typeof(Hub).Assembly);
            ...
            httpServer.OnHTTPResponse += (server, request, response, cancellationToken) => {
                ...
                // traffic log as well - see Hub.Traffic.cs. Wrapped, because
            ...
            // configuration file, because who this Hub is in OCPI is written
            // there. They share the HTTP server above rather than opening a
            // port of their own: OCPI is plain HTTP, and one Hub is one
        ...
        /// they are this Hub's vocabulary and not somebody's data: a group
        ...
        private async Task EnsureAccounts()
        {
            ...
            // Hub that skipped this would find no accounts at every start,
            ...
            if (firstStart)
            {
                ...
                if (organization is not Organization emspOrganization)
                    throw new InvalidOperationException("The organization of this Hub could not be created, and an account outside one cannot sign in.");
                ...
                if (admin is null)
                    throw new InvalidOperationException("The account of this Hub could not be created, so nobody could sign in to it.");
            ...
            foreach (var role in UserRole.All)
            {
                ...
                if (added.Result != CommandResult.Success)
                    throw new InvalidOperationException(
                              $"The user group '{role.GroupId}' of this Hub could not be made: " +
            ...
            if (admin is not null)
            {
                ...
                {
                    throw new InvalidOperationException(
                              $"The account of this Hub could not be put in the {UserRole.SystemAdmin.Name} group, " +
        ...
        /// <summary>
        /// What this Hub is made of, as the Configuration page of the web
        /// interface reads it.
        /// </summary>
        /// <remarks>
