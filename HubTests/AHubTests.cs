...
namespace cloud.charging.open.RoamingHub.Tests
{
    ...
    /// <summary>
    /// One Hub, listening, for the length of one test.
    /// </summary>
    /// <remarks>
    /// A whole Hub per test rather than one for the fixture, because
    /// half of what is worth testing here changes the Hub: a test that
    ...
    /// with a Hub running on 2350 can still run the tests.
    ///
    /// **Nothing here reaches the network.** The configuration written before
    /// the Hub is built switches the time client off, which is what
    ...
    /// </remarks>
    public abstract class AHubTests
    {

        #region Properties

        /// <summary>
        /// The Hub under test, listening, from SetUp until TearDown.
        /// </summary>
        protected Hub  Hub   { get; private set; } = default!;

        /// <summary>
        /// The password this Hub made up for itself at its first start.
        /// </summary>
        ...
        #region What this Hub is made of

        /// <summary>
        /// What its configuration file says before it is built.
        /// </summary>
        /// <remarks>
        /// Overridden by a fixture that needs a Hub with something on
        ...
        public async Task StartTheEMSP()
        {
            ...
            Directory   = TestHubs.TemporaryDirectory("tests");

            Hub  = TestHubs.New(Directory, Configuration, Clock);

            BaseURL     = Hub.WebInterfaceURL.ToString();

            await Hub.Start();

            // After Start(), because that is what makes the account. Null would
            // mean accounts were already there, and the directory is new.
            Password    = Hub.GeneratedPassword
                              ?? throw new InvalidOperationException("The Hub did not make up a password for its first start!");
        ...
        /// <summary>
        /// A browser that has signed in with the password this Hub made
        ...
        protected async Task<HttpClient> SignedIn()
            ...
            => await SignedInAs(Hub.DefaultAdminUser, Password);
        ...
        protected static String SignInPath
            ...
            => $"{Hub.ExtAPIPath.ToString().TrimEnd('/')}/login";
