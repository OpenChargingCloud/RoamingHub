...
namespace cloud.charging.open.RoamingHub
{
    ...
    /// the configuration of this RoamingHub, its log, and one
    ...
    /// </remarks>
    public partial class RoamingHubHTTPAPI : HTTPAPI
    {
        ...
        /// <summary>
        /// Cancelled when this RoamingHub is shutting down, so that the
        /// event streams end.
        /// </summary>
        /// <remarks>
        ...
        /// membership carries this RoamingHub's roles.
        /// </summary>
        ...
        public RoamingHubHTTPAPI(HTTPServer       HTTPServer,
            ...
            : base(HTTPServer,
                   ...
                   Description:  I18NString.Create("The JSON API of this RoamingHub"))

        {
            ...
            this.ExtAPI      = ExtAPI;
            this.Log         = Log;
            this.startedAt   = RoamingHub.TimeProvider.GetUtcNow();

            this.Version     = Version
                                   ?? typeof(RoamingHubHTTPAPI).Assembly.GetName().Version?.ToString(3)
                                   ?? "0.0.0";
        ...
        private void RegisterURLTemplates()
        {
            ...
            // And what went between them; see RoamingHubHTTPAPI.Traffic.cs.
        ...
        /// <summary>
        /// GET /api/v1/status: how this RoamingHub is doing right now.
        /// </summary>
        private Task<HTTPResponse> GetStatus(HTTPRequest Request)
        {
            ...
            var now = RoamingHub.TimeProvider.GetUtcNow();
            ...
            return Task.FromResult(
                       JSONResponse(
                           ...
                           HTTPStatusCode.OK,
                           new JObject(
                               new JProperty("service",    "RoamingHub"),
                               new JProperty("version",    Version),
                               new JProperty("partyId",    RoamingHub.PartyIdText),
        ...
        /// <summary>
        /// GET /api/v1/configuration: what this RoamingHub is made of.
        /// </summary>
        private Task<HTTPResponse> GetConfiguration(HTTPRequest Request)
        {
            ...
            return Task.FromResult(
                       JSONResponse(Request, HTTPStatusCode.OK, RoamingHub.ConfigurationJSON())
                   );
        ...
        /// <summary>
        /// GET /api/v1/configuration/dns: how this RoamingHub resolves names.
        /// </summary>
        private Task<HTTPResponse> GetDNSConfiguration(HTTPRequest Request)
        {
            ...
            return Task.FromResult(
                       JSONResponse(Request, HTTPStatusCode.OK, RoamingHub.DNSConfigurationJSON())
                   );
        ...
        private Task<HTTPResponse> PutDNSConfiguration(HTTPRequest Request)
        {
            ...
            if (!RoamingHub.TryUpdateDNSConfiguration(json, out var error))
        ...
        private async Task<HTTPResponse> PostDNSQuery(HTTPRequest Request)
        {
            ...
            if (!RoamingHub.TryParseRecordTypes(json["recordTypes"], out var recordTypes, out var problem))
                return ErrorJSON(Request, HTTPStatusCode.BadRequest, problem);

            Log.Info($"'{user.Id}' asked this RoamingHub to resolve '{name}'.", "dns", "test", "web");
        ...
        /// <summary>
        /// GET /api/v1/configuration/nts: where this RoamingHub gets the time from.
        /// </summary>
        private Task<HTTPResponse> GetNTSConfiguration(HTTPRequest Request)
        {
            ...
            return Task.FromResult(
                       JSONResponse(Request, HTTPStatusCode.OK, RoamingHub.NTSConfigurationJSON())
                   );
        ...
        private Task<HTTPResponse> PutNTSConfiguration(HTTPRequest Request)
        {
            ...
            if (!RoamingHub.TryUpdateNTSConfiguration(json, out var error))
                return Task.FromResult(ErrorJSON(Request, HTTPStatusCode.BadRequest, error));
        ...
        private async Task<HTTPResponse> PostNTSSync(HTTPRequest Request)
        {
            ...
            var result = await RoamingHub.SyncTimeAsync(Request.CancellationToken);

            var json   = RoamingHub.NTSConfigurationJSON();
        ...
        /// NTS page's Test button of each server sends: "Sync now" asks the
        /// whole group and says what the group concluded, and this is how
        /// somebody finds out where one server of it got to.
        ///
        ...
        private async Task<HTTPResponse> PostNTSTest(HTTPRequest Request)
        {
            ...
            Log.Info($"'{user.Id}' asked this RoamingHub to test {(host is null ? "its time server" : $"the time server '{host}'")}.",
                     "nts", "test", "web");

            return JSONResponse(
                       Request,
                       HTTPStatusCode.OK,
                       await RoamingHub.TestTimeServerAsync(host, Request.CancellationToken)
                   );
        ...
        private Task<HTTPResponse> GetClock(HTTPRequest Request)
        {
            ...
            return Task.FromResult(
                       JSONResponse(Request, HTTPStatusCode.OK, RoamingHub.ClockJSON())
                   );
            ...
            return Task.FromResult(
                       new HTTPResponse.Builder(Request) {
                           ...
                           HTTPSSEWorker   = async (response, stream) => {
                               ...
                               // RoamingHub shutting down ends the stream. The
                               ...
                               try
                               {
                                   ...
                                   // RoamingHub the browser would otherwise wait
        ...
        /// </summary>
        /// <remarks>
        /// Called by <see cref="RoamingHub.Stop"/> before the server is
        ...
        /// this, a RoamingHub with one browser on its Logs page never finishes
        /// shutting down.
        ///
        ...
        /// it has been compared with what the RoamingHub holds, so that refusal
        ...
        /// them find out by being refused. They are a copy of what the RoamingHub
