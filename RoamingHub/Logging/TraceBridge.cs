...
namespace cloud.charging.open.RoamingHub.Logging
{
    ...
    /// <summary>
    /// Everything the libraries below this RoamingHub write with Illias' DebugX,
    /// into the event log - and from there onto the web interface.
    /// </summary>
    /// <remarks>
    /// DebugX writes to <see cref="Debug"/>, and on .NET that reaches
    /// <see cref="Trace.Listeners"/>. So OCPI, Hermod and the rest need not
    /// know this log exists to end up in it: attaching one listener is enough.
    ///
    ...
    /// release build shows on the Logs page is what this RoamingHub logs itself.
    ///
    /// The tags are guessed from the text, by the table in
    /// <see cref="TagsFor"/> - a short, deliberate list of the words that
    /// matter here, and not an attempt to understand the line. A line nothing
    /// matches is tagged "trace" alone, which is also how it can be filtered
    /// away. What a RoamingHub overhears is mostly OCPI going past it in both
    /// directions, so the table leans that way: which module a line is about -
    /// the credentials, the locations, the tokens - is worth more here than
    ...
    public sealed class TraceBridge : TraceListener
    {
        ...
        /// tell what the RoamingHub said itself from what it overheard.
        /// </summary>
