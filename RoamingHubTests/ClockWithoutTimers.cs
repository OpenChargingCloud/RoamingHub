...
namespace cloud.charging.open.RoamingHub.Tests
{
    ...
    /// </summary>
    /// <remarks>
    /// For a test that has to start a hub with its time client switched on -
    /// to read the line a start writes about the clock - and must not have
    /// that hub ask a time server a minute later, which is what the first
    /// clock check would do if the test ever ran that long. That line is
    /// written when the timer is set, not when it fires, so nothing a test
    /// reads is lost.
    ///
