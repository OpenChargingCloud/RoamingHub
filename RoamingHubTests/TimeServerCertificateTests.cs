...
namespace cloud.charging.open.RoamingHub.Tests
{
    ...
    public class TimeServerCertificateTests
    {
        ...
        private static (String Level, String Text)[] Steps(X509Certificate2                    Server,
            ...
            => [.. RoamingHub.TLSSteps(new NTSKE_TLSInfo(
        ...
        public void TheRootCAIsNamedAndFingerprintedForTheList()
        {
            ...
            var json = RoamingHub.RootCAJSON(new NTSKE_TLSInfo(ServerCertificate: server, ValidatedChain: [ server, root ]));
            ...
            Assert.Multiple(() => {
                ...
                Assert.That(RoamingHub.RootCAJSON(null),                 Is.Null);
                Assert.That(RoamingHub.RootCAJSON(new NTSKE_TLSInfo()),  Is.Null, "a session that kept no chain names no root");
            });
